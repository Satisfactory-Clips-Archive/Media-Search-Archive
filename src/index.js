...
(async () => {
	...
	function nudge_visibility() {
		result_count.parentNode.style.visibility = (('' === searchbar.value)
			? 'hidden'
			: 'visible');
	}
	async function merge_search(query, for_dates, qandaonly) {
		...
		for (let i = 0; i < for_dates.length; i++) {
			...
			for (let category in searches_dates[date]) {
				...
				const [docs, index] = await Promise.all(searches_cache[date][category]);
	...
	function perform_search(modify_state = true) {
		...
		timeout = requestAnimationFrame(async () => {
			...
			const filter_date_from = parseInt(formdata.get('df').replace(/\-/g, ''), 10);
			const filter_date_to = parseInt(formdata.get('dt').replace(/\-/g, ''), 10);
			...
			if (modify_state) {
				history.pushState({}, title, `?q=${encodeURIComponent(last_query)}&df=${encodeURIComponent(formdata.get('df'))}&dt=${encodeURIComponent(formdata.get('dt'))}&t=${encodeURIComponent(formdata.get('t'))}&s=${encodeURIComponent(formdata.get('s'))}&o=${encodeURIComponent(formdata.get('o'))}&f=${encodeURIComponent(formdata.get('f'))}&a=${encodeURIComponent(formdata.get('a'))}`);
			}
			const dates_for_search = Object.keys(Object.fromEntries(Object.entries(Object.keys(searches_dates).reduce((out, date) => {
				out[date] = parseInt(date.replace(/\-/g, ''), 10);
				return out;
			}, {})).filter((e) => {
				const [, date_as_int] = e;
				return (date_as_int >= filter_date_from
					&& date_as_int <= filter_date_to);
			})));
			let docs_results = (await merge_search(query.split(' ').map((e) => {
				return e.replace(/\.+$/, '');
			}).join(' '), dates_for_search, qandaonly)).filter((e) => {
				const [doc, result] = e;
				const dateint = parseInt(doc.date.replace(/\-/g, ''), 10);
				return (dateint >= filter_date_from
			...
			if (filter_results) {
				...
				docs_results = docs_results.filter((maybe) => {
					...
					if (id in qandatracking
						&& (((qandatracking[id].duplicatedby
							?? false)
							&& (docs_results_ids.includes(qandatracking[id].duplicatedby ?? '')
								|| docs_results_ids.includes('yt-' + qandatracking[id].duplicatedby)))
							|| ((qandatracking[id].replacedby
								?? false)
								&& (docs_results_ids.includes(qandatracking[id].replacedby ?? '')
									|| docs_results_ids.includes('yt-' + qandatracking[id].replacedby))))) {
						return false;
					}
					return true;
				});
			...
			result_count.textContent = `${docs_results.length} results found matching the term "${query}", under ${'' === filter_topic
				? ' all topics'
				: (' the "'
			...
			if (docs_results.length > 0) {
				...
				docs_results.map((e) => {
					...
					node.querySelector('h1').textContent = (date_format(doc_date)
						+ ' Livestream');
					if ('' !== doc.transcription) {
						const title = node.querySelector('h2');
						title.textContent = '';
						const title_link = document.createElement('a');
						title_link.setAttribute('href', `/transcriptions/${doc.id}`);
					...
					*/
					...
					if ('' !== doc.transcription) {
						...
						transcription.parentNode.parentNode.removeChild(transcription.parentNode);
					}
					if (toggle_transcripts_closed) {
						transcription.parentNode.removeAttribute('open');
					}
					else {
						transcription.parentNode.setAttribute('open', 'open');
					}
					/*
					const positions = [];
	...
	function hashsearch(modify_state = true) {
		...
		if (params.has('q')) {
			...
			if (params.has('t')) {
				const maybe_topic = topic_filter.querySelector(`[value="${params.get('t')}"]`);
				if (maybe_topic) {
	...
	const loading = document.getElementById('loading').content.cloneNode(true);
	const search_result_template = document.getElementById('document-result').content;
	//#region page ready
	const ready = document.getElementById('ready').content.cloneNode(true);
	...
	const sort_by_date_ascending_input = form.querySelector('[name="s"][value="b"]');
	const sort_by_date_input = form.querySelector('[name="s"][value="d"]');
	const toggle_transcripts_closed_input = form.querySelector('[name="o"][value="0"]');
	const toggle_transcripts_open_input = form.querySelector('[name="o"][value="1"]');
	...
	function date_from_value_by_date(date) {
		date_from_value_by_index(date_values_sorted.indexOf(parseInt(date.replace(/\-/g, ''), 10)));
	}
	...
	function date_to_value_by_date(date) {
		date_to_value_by_index(date_values_sorted.indexOf(parseInt(date.replace(/\-/g, ''), 10)));
	}
	...
	[
		...
		const preload = document.head.querySelector(`link[rel="preload"][as="fetch"][href^="${parts[1]}"][href$="${parts[2]}"]`);
		if (!preload) {
	...
	const [qandatracking, searches, synonyms, topics,] = await Promise.all((await Promise.all([
		fetch_qandatracking,
		fetch_searches,
		fetch_synonyms,
		fetch_topics,
	...
	searches_entries.forEach((e) => {
		...
		const date = docs_path.replace(/^docs-[^-]+-(\d{4,}\-\d{2}-\d{2}).+$/, '$1');
		const category = docs_path.replace(/^docs-([^-]+)-(\d{4,}\-\d{2}-\d{2}).+$/, '$1');
	...
	lunr.Pipeline.registerFunction(aliasSatisfactoryVocabulary, 'aliasSatisfactoryVocabulary');
	/*
	...
	*/
	...
	Object.entries(optgroups).forEach((e) => {
		...
		optgroup.appendChild(grouped_topics.reduce((out, topic) => {
			const [slug, unslugged] = topic;
			const option = document.createElement('option');
			option.value = slug;
			option.textContent = ((1 === unslugged.length ? 'All in ' : '')
	...
	document.body.firstElementChild.parentNode.replaceChild(ready, document.body.firstElementChild);
	/**
